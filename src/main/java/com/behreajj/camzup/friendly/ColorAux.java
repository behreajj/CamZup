...
/**
 * Implements color conversion and mixing for Hue, Saturation and Brightness.
 */
public abstract class ColorAux {
    ...
    public static float[] hsbToRgb(final float hue, final float sat, final float val) {
        ...
        final float tint2 = v * (1.0f - s * (h - (float) sector));
        final float tint3 = v * (1.0f - s * (1.0f + (float) sector - h));
        switch (sector) {
            case 0, 6:
    ...
    public static int lerpHsb(final int o, final int d, final float t) {
        ...
        final float oa = (o >> 0x18 & 0xff) * Utils.ONE_255;
        final float or = (o >> 0x10 & 0xff) * Utils.ONE_255;
        final float og = (o >> 0x08 & 0xff) * Utils.ONE_255;
        final float ob = (o & 0xff) * Utils.ONE_255;

        /* Unpack destination color to r, g, b, a. */
        final float da = (d >> 0x18 & 0xff) * Utils.ONE_255;
        final float dr = (d >> 0x10 & 0xff) * Utils.ONE_255;
        final float dg = (d >> 0x08 & 0xff) * Utils.ONE_255;
        final float db = (d & 0xff) * Utils.ONE_255;
        ...
        return (int) (ca * 0xff + 0.5f) << 0x18
            | (int) (rgb[0] * 0xff + 0.5f) << 0x10
            | (int) (rgb[1] * 0xff + 0.5f) << 0x08
            | (int) (rgb[2] * 0xff + 0.5f);
    }
    ...
    public static float[] rgbToHsb(final float r, final float g, final float b) {
        ...
        final float gbmx = Math.max(g, b);
        final float gbmn = Math.min(g, b);
        final float mx = Math.max(gbmx, r);
        if (mx <= 0.0f) {
            return new float[]{0.0f, 0.0f, 0.0f};
        }
        final float mn = Math.min(gbmn, r);
        ...
        return new float[]{hue * Utils.ONE_SIX, diff / mx, mx};
    }
}
