...
public class Mesh2 extends Mesh implements Iterable<Face2>, ISvgWritable {
     ...
     */
    public static final UvProfile.Arc DEFAULT_ARC_UV_PROFILE = UvProfile.Arc.BOUNDS;
    ...
    public static Mesh2 arc(
        ...
        final float oculFac = Utils.clamp(oculus, Utils.EPSILON, 1.0f - Utils.EPSILON);
        if (arcLen1 < 0.00139d) {
        ...
        final float origAngle = Utils.TAU * a1;
        final float destAngle = Utils.TAU * (a1 + arcLen1);

        for (int k = 0; k < sctCount; ++k) {
            final double step = k * toStep;

            final double angle = (1.0d - step) * origAngle + step * destAngle;
        ...
        switch (profile) {
            case CLIP: {
                ...
                for (int k = 0; k < sctCount; ++k) {
                    final double step = k * toStep;
                    final float v = (float) ((1.0d - step) * c1 + step * a1);
                    final int i = k + k;
                    vts[i].set(1.0f, v);
                    vts[i + 1].set(0.0f, v);
                }
            ...
            case STRETCH: {
                for (int k = 0; k < sctCount; ++k) {
                    final float step = (float) (1.0d - k * toStep);
                    final int i = k + k;
                    vts[i].set(1.0f, step);
    ...
    public static Mesh2 circle(final Mesh2 target) {
        ...
        Mesh2.polygon(Mesh.DEFAULT_CIRCLE_SECTORS, Mesh2.DEFAULT_POLY_TYPE, target);
        target.name = "Circle";
        return target;
    }
    ...
    public static Mesh2 circle(final PolyType poly, final Mesh2 target) {
        ...
        Mesh2.polygon(Mesh.DEFAULT_CIRCLE_SECTORS, poly, target);
        target.name = "Circle";
        return target;
    }
    ...
    public static Mesh2 gridHex(
        ...
        final int vRings = Math.max(rings, 1);
        final float vRad = Math.max(Utils.EPSILON, cellRadius);
        final float vMrg = Utils.clamp(margin, 0.0f, vRad - Utils.EPSILON);

        final float extent = Utils.SQRT_3 * vRad;
        final float halfExt = extent * 0.5f;

        final float rad15 = vRad * 1.5f;
        final float padRad = vRad - vMrg;
        final float halfRad = padRad * 0.5f;
        final float radrt32 = padRad * Utils.SQRT_3_2;
        ...
        for (int i = -vRingsn1; i <= vRingsn1; ++i) {
            ...
            for (int j = jMin; j <= jMax; ++j) {
                final float x = iExt + (float) j * halfExt;
                final float y = (float) j * rad15;
     ...
     * @return the merger
     */
    public static Mesh2 groupData(final Collection<Mesh2> coll, final Mesh2 target) {

        return Mesh2.groupData(coll.toArray(new Mesh2[0]), target);
    }
    ...
    public static Mesh2 plane(
        ...
        final int rVal = Math.max(rows, 1);
        final int cVal = Math.max(cols, 1);
        ...
        for (int k = 0; k < fLen1; ++k) {
            final float jStep = (float) (k % cVal1) * jToStep;
            final float iStep = (float) (k / cVal1) * iToStep;
        ...
        switch (poly) {
            ...
            case QUAD: {
                ...
                for (int i = 0, j = 1, k = 2; i < seg; ++i, j += 2, k += 2) {
                    ...
                    vs[k].set((vPrev.x + vNext.x) * 0.5f, (vPrev.y + vNext.y) * 0.5f);

                    final Vec2 vtPrev = vts[j];
                    final Vec2 vtNext = vts[m];
                    vts[k].set((vtPrev.x + vtNext.x) * 0.5f, (vtPrev.y + vtNext.y) * 0.5f);
                }
    ...
    public static Mesh2 square(final PolyType poly, final Mesh2 target) {
        ...
        switch (poly) {
            case NGON:
            case QUAD: {
                target.faces = new int[][][]{{{0, 0}, {1, 1}, {2, 2}, {3, 3}}};
            }
            break;

            case TRI:
            default: {
                target.faces = new int[][][]{{{0, 0}, {1, 1}, {2, 2}}, {{0, 0}, {2, 2}, {3, 3}}};
            }
        }
    ...
    public Mesh2 clean() {
        ...
        final Vec2[] newCoords = coordsTree.toArray(new Vec2[0]);
    ...
    public Mesh2 collapseEdge(
        ...
        this.faces[i] = Mesh.splice(face, j, 2, new int[][]{new int[]{vsOldLen, vtsOldLen}});
    ...
    public Edge2[] getEdgesDirected() {
        ...
        return result.toArray(new Edge2[0]);
    }
    ...
    public Edge2[] getEdgesUndirected() {
        ...
        return result.values().toArray(new Edge2[0]);
    }
    ...
    public Vert2[] getVertices() {
        ...
        return result.toArray(new Vert2[0]);
    }
    ...
    public String toObjString() {
        return this.toObjString(1, 1, 1, true);
    }
    ...
    public static final class Face2Iterator implements Iterator<Face2> {
         ...
         */
        public Face2Iterator(final Mesh2 mesh) {
            this.mesh = mesh;
        }
