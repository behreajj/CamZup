...
public class Knot2 implements Comparable<Knot2> {
     ...
     */
    public Knot2(final Vec2 coord, final Vec2 foreHandle, final Vec2 rearHandle) {
     ...
     */
    public static Knot2 fromSegCatmull(
        final Vec2 prevAnchor,
        final Vec2 currAnchor,
        final Vec2 nextAnchor,
        final Vec2 advAnchor,
        final float tightness,
        final Knot2 prev,
        final Knot2 next) {
    ...
    public static Knot2 smoothHandles(
        ...
        final float rescl = Utils.ONE_THIRD * Utils.invSqrt(xDir * xDir + yDir * yDir);
        carry.x = xDir * rescl;
        carry.y = yDir * rescl;
    ...
    public Knot2 alignHandlesBackward() {
        ...
        if (rmsq > 0.0f) {
            final float flipRescale = -Utils.hypot(this.foreHandle.x - xCoord, this.foreHandle.y - yCoord)
    ...
    public Knot2 alignHandlesForward() {
        ...
        final float fmsq = xFore * xFore + yFore * yFore;
        if (fmsq > 0.0f) {
            final float flipRescale = -Utils.hypot(this.rearHandle.x - xCoord, this.rearHandle.y - yCoord)
    ...
    public Knot2 set(final float xCoord, final float yCoord) {
        ...
        return this.set(xCoord, yCoord, xCoord + xOff, yCoord + yOff, xCoord - xOff, yCoord - yOff);
    }
    ...
    public Knot2 set(
        ...
        this.rearHandle.set(xCoord - (xFore - xCoord), yCoord - (yFore - yCoord));
     ...
     * @return this knot
     */
    public Knot2 set(final Vec2 coord, final Vec2 foreHandle, final Vec2 rearHandle) {
     ...
     */
    public abstract static class AbstrEasing implements Utils.EasingFuncObj<Knot2> {
