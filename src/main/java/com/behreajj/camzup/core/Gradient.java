...
public class Gradient implements Iterable<ColorKey> {
    ...
    public Gradient(final float scalar) {
        ...
        this.keys.add(new ColorKey(Utils.clamp01(scalar), Lab.fromScalar(scalar, new Lab())));
        this.keys.add(new ColorKey(0.0f, Lab.clearBlack(new Lab())));
        this.keys.add(new ColorKey(1.0f, Lab.white(new Lab())));
    }
     ...
     * @param colors the colors
     */
    public Gradient(final Lab... colors) {
        this.appendAll(colors);
    }
    ...
    public static Lab eval(final Gradient grd, final float step, final Lab target) {
        ...
        if (prev == null) {
            return target.set(grd.keys.first().clr);
        }
     ...
     * @return the gradient
     */
    public static Gradient keysByAlpha(final Gradient source, final Gradient target) {
     ...
     * @return the gradient
     */
    public static Gradient keysByLightness(final Gradient source, final Gradient target) {
    ...
    public Gradient appendAll(final Collection<Lab> colors) {
        ...
        final float denom = Utils.div(1.0f, (oldLen + len - 1.0f));
    ...
    public Gradient appendAll(final float... scalars) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            this.keys.add(new ColorKey((oldLen + i) * denom, Lab.fromScalar(scalar, gray)));
        }
    ...
    public Gradient cycle(final int places) {
        ...
        this.keys.clear();
        this.keys.addAll(Arrays.asList(arr).subList(0, len));
    ...
    public Gradient distribute() {
        ...
        this.keys.clear();
        final float denom = Utils.div(1.0f, (keyArr.size() - 1.0f));
    ...
    public Gradient prependAll(final Collection<Lab> colors) {
        ...
        final float denom = Utils.div(1.0f, (this.keys.size() + len - 1.0f));
    ...
    /**
     * Finds this gradient's range. Subtracts the step of its first key from the
     * step of its last.
     *
     * @return the range
     */
    ...
    public boolean removeFirst(final ColorKey target) {
        ...
        final ColorKey first = this.keys.pollFirst();
        if (first != null) {
            target.set(first);
            return true;
        }
    ...
    public boolean removeLast(final ColorKey target) {
        ...
        final ColorKey last = this.keys.pollLast();
        if (last != null) {
            target.set(last);
            return true;
        }
    ...
    public String toSvgString(
        ...
        final String vid = !idTrim.isEmpty() ? idTrim : "camzupGradient";

        final int vw = Math.max(w, 3);
        final int vh = Math.max(h, 3);
        ...
        for (int idx = 0; itr.hasNext(); ++idx) {
            ...
            final float xl = (1.0f - fac0) * swLeft + fac0 * (float) vw;
