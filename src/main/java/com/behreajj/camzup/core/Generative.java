...
public abstract class Generative {
     ...
     * Math.sqrt(2.0)</code>.
     */
    public static final float STEP_2 = Utils.ONE_SQRT_2;

    /**
     * Factor added to 3D noise when returning a Vec3. <code>1.0 /
     * Math.sqrt(3.0)</code>.
     */
    public static final float STEP_3 = Utils.ONE_SQRT_3;
    ...
    /**
     * Returns a signed pseudo-random number in the range [-1.0, 1.0] given a vector
     * input and a seed,
     * based on the vector's hash code.
     *
    ...
    public static float hash(final Vec2 v, final int seed) {
        ...
        return (Float.intBitsToFloat(Generative.hash(v.hashCode(), seed, 0) & 0x007fffff | 0x3f800000)
            - 1.0f)
            * 2.0f
            - 1.0f;
    }
    ...
    public static Vec2 hash(final Vec2 v, final int seed, final Vec2 target) {
        ...
        final float st = Generative.STEP_2 * Utils.sqrtUnchecked(v.x * v.x + v.y * v.y);

        final int ahash = (Utils.MUL_BASE ^ Float.floatToIntBits(v.x + st)) * Utils.HASH_MUL
    ...
    public static Vec3 hash(final Vec3 v, final int seed, final Vec3 target) {
        ...
        final float st = Generative.STEP_3 * Utils.sqrtUnchecked(v.x * v.x + v.y * v.y + v.z * v.z);
        final int mulvx = Utils.MUL_BASE ^ Float.floatToIntBits(v.x);
        final int vybit = Float.floatToIntBits(v.y);
        final int vzbit = Float.floatToIntBits(v.z);

        final int ahash = ((Utils.MUL_BASE ^ Float.floatToIntBits(v.x + st)) * Utils.HASH_MUL ^ vybit)
            * Utils.HASH_MUL
            ^ vzbit;

        final int bhash = (mulvx * Utils.HASH_MUL ^ Float.floatToIntBits(v.y + st)) * Utils.HASH_MUL ^ vzbit;
    ...
    public static Vec4 hash(final Vec4 v, final int seed, final Vec4 target) {
        ...
        final float st = Generative.STEP_4 * Utils.sqrtUnchecked(v.x * v.x + v.y * v.y + v.z * v.z + v.w * v.w);
        final int mulvx = Utils.MUL_BASE ^ Float.floatToIntBits(v.x);
        final int vybit = Float.floatToIntBits(v.y);
        final int vzbit = Float.floatToIntBits(v.z);
