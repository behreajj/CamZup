 ...
 */
public class CurveEntity3 extends Entity3 implements Iterable<Curve3>, IVolume3 {
     ...
     */
    public CurveEntity3() {
        // TODO: Reintroduce method to convert from mesh entity to curve entity?
    }
    ...
    public CurveEntity3 reframe() {
        ...
        while (crvItr.hasNext()) {
            Curve3.accumMinMax(crvItr.next(), lb, ub);
        }

        lb.x = -0.5f * (lb.x + ub.x);
        lb.y = -0.5f * (lb.y + ub.y);
        lb.z = -0.5f * (lb.z + ub.z);
        final float scl = Utils.div(1.0f, Utils.max(ub.x - lb.x, ub.y - lb.y, ub.z - lb.z));
     ...
     * @return the curve
     */
    public Curve3 removeAt(final int i) {
    ...
    public CurveEntity3 toOrigin() {
        ...
        while (itr.hasNext()) {
            Curve3.accumMinMax(itr.next(), lb, ub);
        }
