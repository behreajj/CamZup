...
public interface IUp {
  ...
  /**
   * Converts a an array of characters to a an array of curve entities.
   * When the level of detail is 0, uses Bezier curves; when the detail
   * is non-zero, approximates the glyph with a series of straight line
  ...
  /**
   * Converts a string to a an array of curve entities. When the level
   * of detail is 0, uses Bezier curves; when the detail is non-zero,
  ...
  /**
   * A function to convert a character to an list of curve entities. A
   * helper function for other variants of getGlyph.
   *
  ...
  static List < Curve2 > getGlyph (
    ...
    /*
     * A GlyphVector can handle multiple characters at a time, but
     * individual characters are supplied instead, so that glyphs with
    ...
    /*
     * Acquire an iterator, run through it in a while loop, and deal with
     * 5 possible cases: the 'pen' moves to a point; the 'pen' draws a
     * straight line to the next point; the 'pen' draws a curved line
     * where the fore- and rear-handles are the same; the 'pen' draws a
    ...
    while ( !itr.isDone() ) {
      ...
      switch ( segType ) {
        ...
        case PathIterator.SEG_QUADTO:
          ...
          /*
           * The ordering of the points is (1) a shared handle, or control
           * point, between the previous and next coordinate; (2) the next
           * coordinate.
           */
        ...
        case PathIterator.SEG_CUBICTO:
          ...
          /*
           * The order of a continuing curve is (1) previous knot fore handle;
           * (2) current knot rear handle; (3) current knot point. This is
           * different from a knot constructor's parameter order.
           */
        ...
        case PathIterator.SEG_CLOSE:
          ...
          /*
           * Resolve first and last handles. Not sure why these used to use
           * third pi.
           */
   ...
   * retrieved from a PGraphicsJava2D renderer. The AffineTransform can
   * be null.
   *
   ...
   * Processing uses a deprecated method from an AWT function. That
   * method call has been isolated to this function.
   *
