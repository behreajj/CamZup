...
public abstract class SDF {
   ...
   static {
      HEXAGON = new Vec3(-0.8660254f, 0.5f, 0.57735026f);
   }
   ...
   static float triangle (
      ...
      final float e0x = p1.x - p0.x;
      final float e0y = p1.y - p0.y;

      /* Edge 1 := Corner 2 - Corner 1 */
      final float e1x = p2.x - p1.x;
      final float e1y = p2.y - p1.y;

      /* Edge 2 := Corner 0 - Corner 2 */
      final float e2x = p0.x - p2.x;
      final float e2y = p0.y - p2.y;

      /* Diff 0 := Point - Corner 0 */
      final float v0x = p.x - p0.x;
      final float v0y = p.y - p0.y;

      /* Diff 1 := Point - Corner 1 */
      final float v1x = p.x - p1.x;
      final float v1y = p.y - p1.y;

      /* Diff 2 := Point - Corner 2 */
      final float v2x = p.x - p2.x;
      final float v2y = p.y - p2.y;
   ...
   public static float hexagon (
      ...
      final float dotkp2 = 2.0f * Utils.min(
            SDF.HEXAGON.x * px0 +
                  SDF.HEXAGON.y * py0,
            0.0f);
