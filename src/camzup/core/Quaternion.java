 ...
 * <em>j</em> and <em>k</em>. Discovered by William R. Hamilton with
 * the formula <em>i</em><sup>2</sup> = <em>j</em><sup>2</sup> =
 * <em>k</em><sup>2</sup> = <em>ijk</em> = -1.0 . Quaternions with a
 * magnitude of 1.0 are commonly used to rotate 3D objects from one
 * orientation to another without suffering gimbal lock.
 */
public class Quaternion extends Imaginary implements Comparable < Quaternion > {
   ...
   * equivalence of floating point components, use the static
  ...
  /**
   * Returns -1 when this quaternion is less than the comparisand; 1
   * when it is greater than; 0 when the two are 'equal'. The
   * implementation of this method allows collections of quaternions to
   * be sorted.
   *
   ...
   * retrieving w, or the real component, as the first element.
   *
  ...
  /**
   * Returns a hash code for this quaternion based on its real and
  ...
  /**
   * Attempts to set the components of this quaternion from Strings
   * using {@link Float#parseFloat(String)} . If a NumberFormatException
   * is thrown, the component is set to zero.
   *
  ...
  /**
   * Returns a float array of length 4 containing this quaternion's
   * components. When the argument supplied is true, w is returned as
  ...
  /**
   * Gets the real component. An alias for those accustomed to calling
   * it 'w'.
   *
   ...
   * the quaternion. Uses the formula:<br>
   * <br>
  ...
  /**
   * Raises a quaternion to the power of a real number. Uses the
   * formula<br>
   * <br>
   * <em>a</em><sup><em>b</em></sup> :=
   * |<em>a</em>|<sup><em>b</em></sup> { cos ( <em>b</em> \u03b8 ),
   * <em>n</em> sin ( <em>b</em> \u03b8 ) }<br>
   * <br>
   * where \u03b8 and <em>n</em> are the angle and axis representation
   * of the quaternion <em>a</em>.
   *
  ...
  static Quaternion pow (
    ...
    /*
     * The axis is generated by dividing the imaginary by the arc-sine of
     * the normalized real.
     */
  ...
  /**
   * Returns the conjugate of the quaternion, where the imaginary
   * component is negated.<br>
   * <br>
   ...
   * <br>
   * The dot product of a quaternion with itself is equal to its
   * magnitude squared.
   *
  ...
  /**
   * Sets a quaternion from an angle. The axis is assumed to be (0.0,
   * 0.0, 1.0) . Sets the real component of the quaternion to cosine of
   * the angle; the imaginary z component is set to the sine.
   *
   * Useful when working in 2.5D, where a two-dimensional angle may need
   * to be transferred to a three-dimensional transform.
   *
  ...
  /**
   * Creates a quaternion from three axes - either separate vectors or
   * the columns of a matrix. This is an internal helper function which
   * uses only the relevant information to create a quaternion.
   *
   ...
   * match a pure rotation matrix.
   *
   * @param right   the right axis
   * @param up      the forward axis
   * @param forward the up axis
   * @param target  the output quaternion
   * @return the quaternion
  ...
  /**
   * Sets a quaternion from an axis and angle. Normalizes the axis prior
   * to calculating the quaternion.
   *
   ...
   * creates normalized copies of the vectors. Uses the formula:<br>
   * <br>
   * fromTo (a, b) := { a \u00b7 b, a \u00d7 b }
   *
  ...
  /**
   * Gets the up axis of the rotation. Equivalent to multiplying (0.0,
   * 0.0, 1.0) by the quaternion. If all three axes need to be
   * retrieved, use
   * {@link Quaternion#toAxes(Quaternion, Vec3, Vec3, Vec3)} .
   *
   * @param q      the quaternion
   ...
   * product of a quaternion with itself and to the product of a
   * quaternion with its conjugate.<br>
   * <br>
  ...
  /**
   * Mixes two quaternions together by a step in [0.0, 1.0] with the
  ...
  /**
   * Multiplies two quaternions. Also referred to as the Hamilton
   * product. Uses the formula<br>
   * <br>
  ...
  /**
   * Multiplies two quaternions, then normalizes the product. Emits an
   * unscaled quaternion as an output vector.
   *
   * @param a       the left operand
   * @param b       the right operand
   * @param target  the output quaternion
  ...
  /**
   * Multiplies a vector by a quaternion, in effect rotating the vector
   * by the quaternion. Equivalent to promoting the vector to a pure
   * quaternion, multiplying the rotation quaternion and promoted
   * vector, then dividing the product by the rotation.<br>
   * <br>
   * <em>a</em> <em>b</em> := ( <em>a</em> { 0.0, <em>b</em> } ) / a<br>
   * <br>
   * The result is then demoted to a vector, as the real component
   * should be 0.0 . This is often denoted as <em>P' = RPR'</em>.
   *
   * @param q      the quaternion
  ...
  /**
   * Divides a quaternion by its magnitude, such that its new magnitude
   * is one and it lies on a 4D hyper-sphere. Uses the formula: <br>
   * <br>
  ...
  /**
   * Creates a random unit quaternion. Uses an algorithm by Ken
   * Shoemake, reproduced at this Math Stack Exchange discussion:
   * <a href=
  ...
  public static Quaternion rotate (
    ...
    final float mSq = Quaternion.magSq(q);
    if (mSq == 0.0f) { return Quaternion.fromAxisAngle(
        radians, axis, target); }
   ...
   *
   * Do not use sequences of ortho-normal rotations by Euler angles;
   * this will result in gimbal lock, defeating the purpose behind a
   * quaternion.
   *
   * @param q       the input quaternion
   * @param radians the angle in radians
   ...
   * will result in gimbal lock, defeating the purpose behind a
   * quaternion.
   *
   * @param q       the input quaternion
   * @param radians the angle in radians
  ...
  /**
   * Rotates a vector around the z axis. Accepts calculated sine and
   * cosine of half the angle so that collections of quaternions can be
   * efficiently rotated without repeatedly calling cos and sin.
   *
   * @param q      the input quaternion
  ...
  /**
   * Subtracts the right quaternion from the left and normalizes the
   * result. Emits the difference as an output vector.
   *
   * @param a      the left operand
   * @param b      the right operand
   * @param target the output quaternion
   ...
   * rotation matrix.
   *
   * Use this instead of getRight, getForward and getUp if all three
   * axes are needed.
   *
   * @param q       the quaternion
   * @param right   the right axis
  ...
  /**
   * Converts a quaternion to an axis and angle. The angle is returned
   * from the function. The axis is assigned to an output vector.
   *
   * @param q    the quaternion
