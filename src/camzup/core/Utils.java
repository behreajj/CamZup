...
/**
 * Implements basic math utilities for single- and double-precision
 * numbers which may serve as alternatives to Math library functions.
 */
public abstract class Utils implements IUtils {
  ...
  /**
   * A bounds-checked approximation of the arc-cosine for single
   * precision real numbers. Returns a value in the range [0.0, \u03c0]:
   * \u03c0, when the input is -1.0; \u03c0 / 2.0, when the input is
   * 0.0; 0.0, when the input is 1.0.<br>
   * <br>
   * An alternative to {@link Math#acos(double)} . Based on the
   * algorithm at the <a href=
   * "https://developer.download.nvidia.com/cg/acos.html">Nvidia Cg 3.1
   * Toolkit Documentation</a>. This cites M. Abramowitz and I.A.
   * Stegun, Eds., <em>Handbook of Mathematical Functions</em>, possibly
   * p. 83, which cites <em>Approximations for Digital Computers</em> by
   * C. Hastings, Jr.
   *
   * @param value the input value
   ...
   * difference between two values is less than an epsilon. Does not
   * handle edge cases.
   *
   * @param a the left operand
   * @param b the right operand
  ...
  /**
   * A quick approximation test. Tests to see if the absolute of the
   * difference between two values is less than a tolerance. Does not
   * handle edge cases.
   *
   * @param a         the left operand
   * @param b         the right operand
  ...
  /**
   * A bounds-checked approximation of the arc-sine for single precision
   * real numbers. Returns a value in the range [-\u03c0 / 2.0, \u03c0 /
   * 2.0]: -\u03c0 / 2.0, when the input is -1.0; 0.0, when the input is
   * 0.0; \u03c0 / 2.0, when the input is 1.0.<br>
   * <br>
   * An alternative to {@link Math#asin(double)} . Based on the
   * algorithm at the <a href=
   * "https://developer.download.nvidia.com/cg/asin.html">Nvidia Cg 3.1
   * Toolkit Documentation</a> . This cites M. Abramowitz and I.A.
   * Stegun, Eds., <em>Handbook of Mathematical Functions</em>, possibly
   * p. 83, which cites <em>Approximations for Digital Computers</em> by
   * C. Hastings, Jr.
   ...
   * <em>precedes</em> the horizontal. The return value falls in the
   * range [-\u03c0, \u03c0] .
   *
   * An alternative to {@link Math#atan2(double, double)} . Based on the
   * algorithm at the <a href=
   * "https://developer.download.nvidia.com/cg/atan2.html">Nvidia Cg 3.1
  ...
  public static float atan2 ( final float y, final float x ) {
    ...
    /*
     * When stored independently as floats, some of these magic numbers
     * are truncated to slightly different values.
     */
   ...
   * denominator is zero, avoiding the java.lang.ArithmeticException .
   * This is to simulate the convention of shading languages like GLSL
   * and OSL.
   *
  ...
  /**
   * A fast inverse square-root. Returns 0.0 when the value is less then
   * or equal to zero. Use the unchecked version when the input value is
  ...
  /**
   * The inverse square root implementation based on the 'evil bit hack'
   * from Quake 3, as described by Chris Lomont in
   * "<a href= "http://www.lomont.org/papers/2003/InvSqrt.pdf">Fast
   * Inverse Square Root</a>." For accuracy, the result is refined three
  ...
  /**
   * Linear interpolation from the origin to the destination value by a
   * step. If the step is less than zero, returns the origin. If the
   * step is greater than one, returns the destination.
   *
   * @param origin the origin value
   * @param dest   the destination value
   * @param step   the step
  ...
  /**
   * Linear interpolation from the origin to the destination value by a
   * step. Does not check to see if the step is beyond the range [0.0,
   * 1.0] .
   *
   * @param origin the origin value
   * @param dest   the destination value
  ...
  /**
   * Applies floorMod to the operands, and therefore uses the formula
   * mod ( a, b ) := a - b * floor ( a / b ) .
   *
   * If the right operand is one, use a - floor(a) instead.
   *
   * If the right operand is zero, returns the left operand instead of
   * throwing a java.lang.ArithmeticException for division by zero.
   *
   * @param a the left operand
   * @param b the right operand
   * @return the result
  ...
  /**
   * An alternative to {@link Math#floorMod(int, int)} . Returns a when
   * b is zero.
   *
   * @param a the left operand
   * @param b the right operand
  ...
  /**
   * Subtracts the floor of the input value from the value. Use this
   * instead of {@link Utils#mod(int, int)} when b is 1.0 .
   *
   * @param value the input value
   * @return the result
   */
  ...
  /**
   * A specialized version of mod which shifts an angle in degrees to
   * the range [0, 360]. Casts float input to a double, uses a constant
   * for 1.0 / 360, then casts back to a float upon return.
   *
  ...
  /**
   * A specialized version of mod which shifts an angle in radians to
   * the range [0, \u03c4] .
   *
   * @param radians the angle in radians
   ...
   * mod ( a, b ) := a - b * floor ( a / b ) .
   *
   * If the right operand is one, use mod ( a ) = a - floor ( a )
   * instead.
   *
  ...
  /**
   * The negation of a float holding a boolean value. Given a == 1.0, !a
   * == 0 . Given a == 0.0, !a == 1 .
   *
   * @param value the input value
   * @return the negation
  ...
  /**
   * Evaluates two floats like booleans, using the analytic definition
   * of the inclusive or (OR) logic gate.
   *
   * @param a the left operand
   * @param b the right operand
   * @return the evaluation
  ...
  public static float quantize ( final float value, final int levels ) {
     ...
     * round(m x n - 0.5) / (n - 1), should not be used. It will yield
     * output values exceeding the magnitude of input values, e.g. 1.0
     * will return 2.0 .
     */
  ...
  /**
   * Rounds a value to an integer based on whether its fractional
   * portion is greater than or equal to plus or minus 0.5 .
   *
   * @param value the value
   ...
   * angle with single precision real numbers. The radians supplied to
   * this function should be normalized through division by TAU.
   * Subtract 0.25 from the input value to return the sine instead of
   * the cosine.<br>
   * <br>
   ...
   * value is less than zero. An alternative to
   * {@link Math#sqrt(double)} . Use {@link Utils#sqrtUnchecked(float)}
   * when the value is known to be positive. Use
   * {@link Complex#sqrt(float, Complex)} when the input may be negative
  ...
  /**
   * Equivalent to dividing the sine of the angle by the cosine. Finds
   * the approximate tangent of an angle in radians. An alternative to
   * the double precision {@link Math#tan(double)} , this function uses
  ...
  /**
   * Converts a float to a boolean, where 0.0 and NaN are false; all
  ...
  /**
   * Converts an integer to a boolean, where 0 is false; all other
   * values are true.
   *
   * @param value the input value
   * @return the boolean
  ...
  /**
   * A quick, dirty representation of a single-precision real number as
   * a String to a number of places. Truncates the final digit right of
   * the decimal place. Edge cases:
   * <ul>
   * <li>When the number of places is less than one, returns the String
   ...
   * DecimalFormat, which extrapolates values beyond the last decimal
   * place.
   *
  ...
  /**
   * Converts a boolean value to a float, where 1.0 is true and 0.0 is
   * false.
   *
  ...
  /**
   * Converts a boolean value to an integer, where 1 is true and 0 is
   * false.
   *
   * @param bool the input boolean
  ...
  /**
   * Returns a String representation of a two dimensional array of
   * objects.
   *
   * @param arr the array
   * @return the String
  ...
  /**
   * Returns a String representation of a three dimensional array of
   * objects.
   *
   * @param arr the array
   * @return the String
  ...
  /**
   * Truncates the input value. This is an alias for explicitly casting
   * a float to an integer, then implicitly casting the integral to a
   * float.
   *
   * @param value the input value
   * @return the integral
  ...
  /**
   * Wraps a value around a periodic range as defined by an upper and
   * lower bound: lower bounds inclusive; upper bounds exclusive. Due to
   * single precision accuracy, results will be inexact.
   *
   * In cases where the lower bound is greater than the upper bound, the
   * two will be swapped. In cases where the range is 0.0, 0.0 will be
   * returned.
   *
   * @param value the value
   * @param lb    the lower bound
  ...
  /**
   * Evaluates two floats like booleans, using the analytic definition
   * of the exclusive or (XOR) logic gate.
   *
   * @param a the left operand
   * @param b the right operand
   * @return the evaluation
