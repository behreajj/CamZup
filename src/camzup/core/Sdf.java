...
/**
 * Facilitates implicit shapes created with signed distance fields.
 * Adapted from the GLSL of Inigo Quilez: <a href=
...
public abstract class Sdf {
  ...
  /**
   * Draws an open arc with rounded stroke caps. The angular offset of
   * the arc's aperture is to be calculated outside of the function. The
   * same goes for <em>twice</em> the arc-length of the arc's aperture.
   * Based on the GLSL: <a href="https://www.shadertoy.com/view/wl23RK">
  ...
  /**
   * A wrapper function around the default signed distance for arc.
   * Follows the Processing convention of specifying the start and stop
   * angle as inputs. This calls cosine twice and sine twice, and so
  ...
  /**
   * Draws a two dimensional box with rounded corners whose dimensions
   * are described by the bounds.
   *
   * @param point    the point
   * @param bounds   the bounds
  ...
  /**
   * Draws a three dimensional box with rounded corners whose dimensions
   * are described by the bounds.
   *
   * @param point    the point
   * @param bounds   the bounds
  ...
  public static float conic (
    ...
    /*
     * Judging by the appearance of ZImage gradients made with this
     * function, Math#atan2 with double precision is necessary.
     */
  ...
  /**
   * Finds the intersection between two shapes as represented by
   * factors.
   *
  ...
  /**
   * Draws a polygon from a series of vertices. The number of vertices
   * is assumed to be greater than three. With reference to
  ...
  public static float polygon (
     ...
     * does not need to be calculated prior to the for-loop. d will be
     * replaced by any lesser value, so it makes sense to start with
     * MAX_VALUE instead.
     */
  ...
  /**
   * Draws a rounded polygon from an array of vertices, assumed to be
   * greater than two.
   *
   * @param point    the point
   * @param vertices the vertices
  ...
  /**
   * Finds the rounded subtraction of two shapes as represented by
   * factors.
   *
   * @param a      the left factor
   * @param b      the right factor
