...
public abstract class Mesh extends EntityData implements IMesh {
   ...
   * mesh; 3D meshes, for example, include normals while 2D meshes
   * don't.
   */
  public int[][][] faces;
  ...
  /**
   * Cycles the array of indices in the faces array by a number of
   * places. The number of places can be positive or negative,
   * indicating which direction to shift the array: positive numbers
   ...
   * therefore edge) order by a number of places. The number of places
   * can be positive or negative, indicating which direction to shift
   * the array: positive numbers shift to the right; negative, to the
   * left.
   *
  ...
  protected static int[][] insert (
     ...
     * (2.) Copy insertion into result array at the insertion point. (3.)
     * Copy portion of source array after the insertion point into the
