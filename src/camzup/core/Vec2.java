 ...
 * Processing's PVector. This is intended for storing points and
 * directions in two-dimensional graphics programs. Instance methods
...
public class Vec2 extends Vec implements Comparable < Vec2 > {
   ...
   * value for use by SVG formatting functions.
   *
   * @return the string
   */
  String toSvgString ( ) {
    ...
    /*
     * With precision of 4, SVG rendering encountered glitches due to
     * single-precision values being scaled up.
     */
   ...
   * index is 1 or -1, returns y; 0 or -2, x.
   *
  ...
  /**
   * Returns a float array of length 2 containing this vector's
   * components.
   *
   * @return the array
   */
   ...
   * in row-major order, but the parameters are supplied in reverse:
  ...
  /**
   * Returns a point on a Bezier curve described by two anchor points
   * and two control points according to a step in [0.0, 1.0] .
   *
   * When the step is less than one, returns the first anchor point.
   * When the step is greater than one, returns the second anchor point.
   *
  ...
  /**
   * Returns a tangent on a Bezier curve described by two anchor points
   * and two control points according to a step in [0.0, 1.0] .
   *
   * When the step is less than one, returns the first anchor point
   * subtracted from the first control point. When the step is greater
   * than one, returns the second anchor point subtracted from the
   ...
   * mathematically incorrect, but serves as a shortcut for transforming
   * a vector by the inverse of a scalar matrix.
   *
   * @param a      numerator
   * @param b      denominator
   * @param target the output vector
  ...
  /**
   * Flattens a two dimensional array of vectors to a one dimensional
   * array.
   *
  ...
  /**
   * Creates a vector from polar coordinates: (1) theta, \u03b8, an
   * angle in radians, the vector's heading; (2) rho, \u03c1, a radius,
   * the vector's magnitude. Uses the formula<br>
   * <br>
   * ( \u03c1 cos ( \u03b8 ),<br>
   * \u03c1 sin ( \u03b8 ) ).
   *
  ...
  /**
   * Creates a vector with a magnitude of 1.0 from an angle, such that
   * the vector is on the unit circle.
   *
   * @param heading the angle in radians
   * @param target  the output vector
   * @return the vector
  ...
  /**
   * Generates a 2D array of vectors. Defaults to the coordinate range
   * of [-0.5, 0.5] .
   *
  ...
  /**
   * Generates a 2D array of vectors. The result is in row-major order,
   * but the parameters are supplied in reverse: columns first, then
   ...
   * coordinates, rows are staggered by an offset. In array storage, the
   * grid uses an axial coordinate system: column-minor coordinates
   * advance diagonally rather than in a jagged vertical.
   *
  ...
  public static Vec2[][] gridHex (
     ...
     * Math.sqrt(3.0d) / 8.0d = 0.21650635094610965d . This is the
     * constant Math.sqrt(3.0d) / 2.0d multiplied by the expected radius,
     * 0.5d, then halved, so that the x offset can be both positive and
     * negative.
     */
  ...
  /**
   * Generates a 2D array of vectors. The array is ordered by rings,
   * then sectors; the parameters are supplied in reverse order.
   *
  ...
  /**
   * Inserts an array of vectors in the midst of another. The insertion
   * point is before, or to the left of, the existing element at a given
   * index.
   *
   * @param arr    the array
  ...
  /**
   * Tests to see if the vector is on the unit circle, i.e., has a
   ...
   * referred to as the radius when using polar coordinates. Uses the
   * formula \u221a <em>a</em> \u00b7 <em>a</em> . Where possible, use
   ...
   * vector from the vector. For Vec2s, useful for managing texture
   ...
   * unit vector, as it lies on the circumference of a unit circle.
   *
   * @param v      the input vector
   * @param target the output vector
  ...
  /**
   * Finds the perpendicular of a vector in the counter-clockwise
   * direction, such that
   * <ul>
   *
  ...
  /**
   * Finds the perpendicular of a vector in the clockwise direction,
   * such that
   * <ul>
   *
  ...
  /**
   * Generates a vector with a random heading and a magnitude of 1.0,
   * such that it lies on the unit circle.
   *
  ...
  /**
   * Creates a vector with a magnitude of 1.0 at a random heading, such
  ...
  /**
   * Resizes an array of vectors to a requested length. If the new
   * length is greater than the current length, the new elements are
   * filled with new vectors. If the new length equals the old, the
   ...
   *
   * For 2D vectors, this scales the y component by cosine of the angle.
   *
   * @param v       the input vector
   ...
   *
   * For 2D vectors, this scales the x component by cosine of the angle.
   *
   * @param v       the input vector
   * @param radians the angle in radians
   * @param target  the output vector
   ...
   * cosine of an angle, so that collections of vectors can be
