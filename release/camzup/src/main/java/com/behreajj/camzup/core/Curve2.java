...
public class Curve2 extends Curve implements Iterable<Knot2>, ISvgWritable {
     ...
     */
    private final ArrayList<Knot2> knots = new ArrayList<>(Curve.KNOT_CAPACITY);

    /**
     * The default constructor.
     */
    public Curve2() {
        // TODO: Reintroduce method to create curve from mesh face.
    }

    /**
     * Creates a curve from a collection of knots
     *
     * @param cl    whether the curve is closed
     ...
     * @param knots the collection of knots
     */
    public Curve2(final String name, final boolean cl, final Collection<Knot2> knots) {
    ...
    public static Curve2 arc(
         ...
         * arc length is near 90, 180, 270 or 360 degrees, then an extra knot
         * is not needed.
         */
        ...
        if (target.closedLoop) {
            ...
            if (arcMode == ArcMode.CHORD) {
                ...
                last.foreHandle.set(
                    Utils.TWO_THIRDS * coLast.x + Utils.ONE_THIRD * coFirst.x,
                    Utils.TWO_THIRDS * coLast.y + Utils.ONE_THIRD * coFirst.y);

                first.rearHandle.set(
                    Utils.TWO_THIRDS * coFirst.x + Utils.ONE_THIRD * coLast.x,
                    Utils.TWO_THIRDS * coFirst.y + Utils.ONE_THIRD * coLast.y);
                ...
                final float cox23 = Utils.TWO_THIRDS * coCenter.x;
                final float coy23 = Utils.TWO_THIRDS * coCenter.y;
                final float cox13 = Utils.ONE_THIRD * coCenter.x;
                final float coy13 = Utils.ONE_THIRD * coCenter.y;
     ...
     */
    public static Bounds2 calcBounds(final Curve2 curve, final Bounds2 target) {
    ...
    public static Curve2 circle(final Curve2 target) {
        ...
        return Curve2.circle(Curve.KNOTS_PER_CIRCLE, target);
    }
     ...
     */
    public static Ray2 eval(final Curve2 curve, final float step, final Ray2 ray) {
    ...
    public static Vec2 eval(
        ...
        if (curve.closedLoop) {
            ...
            if (step >= 1.0f) {
                return Curve2.evalLast(curve, coord, tangent);
            }
    ...
    public static Curve2 fromQuadratic(
        ...
        final int ptsLen = points.length;
        if (ptsLen < 3) {
            return Curve2.fromPoints(false, points, target);
        }
        final int knotCount = ptsLen / 2 + (closedLoop ? 0 : 1);
        if (knotCount < 2 || (closedLoop ? ptsLen % 2 != 0 : (ptsLen + 1) % 2 != 0)) {
    ...
    public static Curve2 infinity(final Curve2 target) {
        ...
        itr.next().set(0.235709f, 0.166627f, 0.0505335f, 0.114256f, 0.361728f, 0.2022675f);
        itr.next().set(-0.235709f, -0.166627f, -0.361728f, -0.2022675f, -0.0505335f, -0.114256f);
        itr.next().set(-0.5f, 0.0f, -0.5f, 0.1309615f, -0.5f, -0.1309615f);
        itr.next().set(-0.235709f, 0.166627f, -0.0505335f, 0.114256f, -0.361728f, 0.2022675f);
        itr.next().set(0.235709f, -0.166627f, 0.361728f, -0.2022675f, 0.0505335f, -0.114256f);
     ...
     * @return the rectangle
     */
    public static Curve2 rect(final Vec2 tl, final Vec2 br, final Curve2 target) {
     ...
     */
    public static Curve2 rect(final Vec2 tl, final Vec2 br, final float corner, final Curve2 target) {
    ...
    static Curve2 circle(
        ...
        final float offNorm = offsetAngle * Utils.ONE_TAU;
        final int vKnCt = Math.max(sectors, 3);
        target.resize(vKnCt);
    ...
    static Curve2 ellipse(
        ...
        final float horizHandle = vrad * Curve.KAPPA;
        final float vertHandle = ry * Curve.KAPPA;
    ...
    static Vec2 lerp13(final Vec2 a, final Vec2 b, final Vec2 target) {
        ...
        return target.set(
            Utils.TWO_THIRDS * a.x + Utils.ONE_THIRD * b.x,
            Utils.TWO_THIRDS * a.y + Utils.ONE_THIRD * b.y);
    }
    ...
    static Curve2 rect(
        ...
        /* Validate corners. */
        float lft = Math.min(lbx, ubx);
        float btm = Math.min(lby, uby);
        float rgt = Math.max(ubx, lbx);
        float top = Math.max(uby, lby);

        /* Protect against zero dimensions on w or h or both. */
        float w = rgt - lft;
        float h = top - btm;
        final boolean wInval = w < Utils.EPSILON;
        final boolean hInval = h < Utils.EPSILON;
        if (wInval && hInval) {
        ...
        final float vbl = Math.min(se, Utils.abs(bl));
        final float vbr = Math.min(se, Utils.abs(br));
        final float vtr = Math.min(se, Utils.abs(tr));

        /* To extend corner handles. */
        final float vtlk = vtl * Curve.KAPPA;
        final float vblk = vbl * Curve.KAPPA;
        final float vbrk = vbr * Curve.KAPPA;
        final float vtrk = vtr * Curve.KAPPA;

        /* To lerp handles on flat edges. */
        final float t = Utils.ONE_THIRD;
        final float u = Utils.TWO_THIRDS;
        ...
        /* Top left corner. */
        if (tl > 0.0f) {
            itr.next().set(lftIns0, top, lftIns0 - vtlk, top, u * lftIns0 + t * rgtIns0, top);
            itr.next().set(lft, topIns1, lft, u * topIns1 + t * btmIns1, lft, topIns1 + vtlk);
        } else if (tl < -0.0f) {
            itr.next().set(lftIns0, top, lftIns0, top - vtlk, u * lftIns0 + t * rgtIns0, top);
            itr.next().set(lft, topIns1, lft, u * topIns1 + t * btmIns1, lft + vtlk, topIns1);
        } else {
            itr.next().set(lft, top, lft, u * top + t * btmIns1, u * lft + t * rgtIns0, top);
        }

        /* Bottom left corner. */
        if (bl > 0.0f) {
            itr.next().set(lft, btmIns1, lft, btmIns1 - vblk, lft, u * btmIns1 + t * topIns1);
            itr.next().set(lftIns1, btm, u * lftIns1 + t * rgtIns1, btm, lftIns1 - vblk, btm);
        } else if (bl < -0.0f) {
            itr.next().set(lft, btmIns1, lft + vblk, btmIns1, lft, u * btmIns1 + t * topIns1);
            itr.next().set(lftIns1, btm, u * lftIns1 + t * rgtIns1, btm, lftIns1, btm + vblk);
        } else {
            itr.next().set(lft, btm, u * lft + t * rgtIns1, btm, lft, u * btm + t * topIns1);
        }

        /* Bottom right corner. */
        if (br > 0.0f) {
            itr.next().set(rgtIns1, btm, rgtIns1 + vbrk, btm, u * rgtIns1 + t * lftIns1, btm);
            itr.next().set(rgt, btmIns0, rgt, u * btmIns0 + t * topIns0, rgt, btmIns0 - vbrk);
        } else if (br < -0.0f) {
            itr.next().set(rgtIns1, btm, rgtIns1, btm + vbrk, u * rgtIns1 + t * lftIns1, btm);
            itr.next().set(rgt, btmIns0, rgt, u * btmIns0 + t * topIns0, rgt - vbrk, btmIns0);
        } else {
            itr.next().set(rgt, btm, rgt, u * btm + t * topIns0, u * rgt + t * lftIns1, btm);
        }

        /* Top right corner. */
        if (tr > 0.0f) {
            itr.next().set(rgt, topIns0, rgt, topIns0 + vtrk, rgt, u * topIns0 + t * btmIns0);
            itr.next().set(rgtIns0, top, u * rgtIns0 + t * lftIns0, top, rgtIns0 + vtrk, top);
        } else if (tr < -0.0f) {
            itr.next().set(rgt, topIns0, rgt - vtrk, topIns0, rgt, u * topIns0 + t * btmIns0);
            itr.next().set(rgtIns0, top, u * rgtIns0 + t * lftIns0, top, rgtIns0, top - vtrk);
        } else {
            itr.next().set(rgt, top, u * rgt + t * lftIns0, top, rgt, u * top + t * btmIns0);
        }
    ...
    public Curve2 reframe(final Transform2 tr) {
        ...
        lb.x = 0.5f * (lb.x + ub.x);
        lb.y = 0.5f * (lb.y + ub.y);
        final float scl = Utils.div(1.0f, Math.max(dim.x, dim.y));
     ...
     */
    public static class Easing implements Utils.EasingFuncArr<Curve2>, Utils.EasingFuncObj<Curve2> {
        ...
        public Curve2 applyUnclamped(
            ...
            if (orKn.size() == dsKn.size() && orig.closedLoop == dest.closedLoop) {
                ...
                while (orItr.hasNext() && dsItr.hasNext()) {
                    this.easingFunc.apply(orItr.next(), dsItr.next(), step, tgItr.next());
                }
            }
