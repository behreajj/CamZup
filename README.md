...
- Processing pre-calculates the values of cosine and sine, then stores them in a [lookup table](https://en.wikipedia.org/wiki/Lookup_table#Computing_sines) (LUT); this is a common technique to speed up the `cos` and `sin` functions in the draw loop. The `pfriendly` package uses `PApplet.cos` and `sin`. The `core` uses a single precision approximation except where the precision of `Math` is needed.
